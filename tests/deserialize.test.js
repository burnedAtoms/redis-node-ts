...
test("Error string with colon inside should return full error message", () => {
    expect((0, deserialize_1.Deserialize)("-Error:Something went wrong")).toStrictEqual(["Error:Something went wrong"]);
});
test("Hard nested RESP message with mixed types", () => {
    ...
    expect((0, deserialize_1.Deserialize)(input)).toStrictEqual(expected);
});
...
test("Incomplete bulk string missing trailing CRLF", () => {
    ...
    expect((0, deserialize_1.Deserialize)(input)).toStrictEqual(["abc"]); // or expect(Deserialize(input)).toStrictEqual(["abc"]);
});
